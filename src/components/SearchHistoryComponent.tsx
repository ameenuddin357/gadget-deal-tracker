...
export const SearchHistoryComponent: React.FC<SearchHistoryComponentProps> = ({
  ...
  useEffect(() => {
    ...
    async function loadHistory() {
      ...
      setError(null);
      try {
        const dbHistory = await getDetailedSearchHistory();
        if (active) {
          const terms = Array.from(new Set(dbHistory.map((item: any) => item.search_term || item.query).filter(Boolean))) as string[];
          setHistory(terms);
          // Sync local storage as backup fallback
          localStorage.setItem('deal_search_history', JSON.stringify(terms));
        }
      } catch (err: any) {
          ...
          try {
            ...
            if (saved) {
              ...
              if (Array.isArray(parsed)) {
                // Handle cases where older cached data might contain objects
                const terms = Array.from(new Set(parsed.map((item: any) => 
                  typeof item === 'string' ? item : (item.search_term || item.query)
