...
export const getWatchlist = async (
  ...
  try {
    ...
    const enrichedWatchlist = result.rows.map((r: any) => ({
      ...r,
    ...
    res.status(200).json({
      status: 'success',
      results: enrichedWatchlist.length,
      data: {
...
export const addToWatchlist = async (
  ...
  try {
    ...
    const productCheck = await db.query('SELECT product_id FROM products WHERE product_id = $1;', [pId]);
    ...
    const result = await db.query(sql, [userId, pId]);
...
export const removeFromWatchlist = async (
  ...
  try {
    ...
    if (isNaN(watchlistId) || watchlistId <= 0 || watchlistId > 2147483647) {
